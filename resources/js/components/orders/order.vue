  ...
  <div class="row">
    ...
    <div class="col-4 order-container">
      <div class="order-inner-container"> 
        ...
        <!-- Edit Status -->
        <div v-show="editStatus" id="order-statuses-edit">
          <edit-status 
            :current-status="order.statuses != undefined ? order.statuses[0].id : null"
            :order-id="id"
            @status-edited="statusEdited"
          ></edit-status>
        </div>
        <!-- Current Status -->
        <div class="row justify-content-between m-0">
          <span v-if="statuses && statuses[0]" class="align-self-center" >
            {{statuses[0].name}}
          </span>
          <a class="" data-toggle="collapse" href="#order-statuses" role="button" aria-expanded="false" aria-controls="collapseExample">
            <font-awesome-icon icon="bars" size="2x"/> 
          </a>       
        </div>       
        <!-- Status History -->
        <div class="order-statuses collapse" id="order-statuses">
          <div v-if="statusesLoad">
