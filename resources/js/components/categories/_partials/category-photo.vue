...
<div>
  ...
  <b-button v-b-modal.product-main-photo-modal>Редактировать Фото</b-button>

  <div class="new-main-photo my-2 d-flex">

    <!-- Current main photo -->
    <img :src="category.mainImage" style="width:75px; height:75px">
  ...
  <b-modal v-model="modalShow" size="lg" id="product-main-photo-modal" title="Редактирование главного фото" ok-only>
      ...
      <div class="photo-list row">
        <img v-for='(image,i) in category.images' @click="edit(image)" class="m-2 border" :key='i' :src="image" >
      </div>
  ...
  methods:{
    ...
    async save(){
      ...
      canvas.toBlob(async (blob) => {
        ...
        r = await ax.fetch('/categoty/main/photo',{file:r,categoryId:this.category.id},'post');
