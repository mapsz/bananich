...
<form class="juge-form" @submit.prevent="submit()">
  ...
  <template v-for="(input,i) in inputs">
    ...
    <!-- Label -->
    <span class="juge-form-label-required" :key="i">      
      <!-- Required -->
    ...
    <template v-if="input.multi == undefined || input.multi == false">
      <juge-input v-model="data[input.name]" :input="input" :key="i+'a'"></juge-input>
      <!-- info -->
      <span v-if="input.multi == undefined || !input.multi" :key="i+'b'">
